 ...
 *
 * <p>
 * Implements the AE2 {@link ICellHandler} interface to provide custom cell inventories for
...
public class EStorageCellHandler implements ICellHandler {
    ...
    @Override
    public void openChestGui(EntityPlayer player, IChestOrDrive chest, ICellHandler handler, IMEInventoryHandler inv,
        ItemStack cell, StorageChannel channel) {
    ...
    public int getStatusForCell(ItemStack cell, IMEInventory handler) {
        ...
        if (handler instanceof EStorageCellInventory.EStorageCellInventoryHandler) {
            IMEInventory<appeng.api.storage.data.IAEItemStack> internal = ((EStorageCellInventory.EStorageCellInventoryHandler) handler)
                .getInternal();
